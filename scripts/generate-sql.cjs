...
let sql = `INSERT INTO events (title, date, category, "picName", "picPhone", status) VALUES\n`;
const values = [];
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (parts.length >= 5) {
    ...
    if (tipePeriode.toLowerCase() === 'bulanan') {
      ...
      values.push(`('${title}', '${targetDate}', '${category}', '${picName}', '${picPhone}', 'pending')`);
    }
  }
}

sql += values.join(',\n') + ';';
fs.writeFileSync('insert_rutinan.sql', sql);
console.log('SQL file generated at insert_rutinan.sql');
